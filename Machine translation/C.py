            ...
            for sent in aligned_sents:
                ...
                for i in range(0,m):
                    for j in range(0,l):
                        ...
                        sum_delta2 = 0.0
                        for h in range(l):
                            sum_delta1 = sum_delta1 + q_fe[(h,i,l,m)]*t_fe[(french_words[i],eng_words[h])]
                            sum_delta2 = sum_delta2 + q_ef[(i,h,m,l)]*t_ef[(eng_words[h],french_words[i])]
                    #sum_q_fe[i]= sum(q_fe_list)
                    #sum_q_ef[i] = sum(q_ef_list)
                
                    
                        delta1 = q_fe[(j,i,l,m)]*t_fe[(french_words[i],eng_words[j])]/ float(sum_delta1)
                        delta2 = q_ef[(i,j,m,l)]*t_ef[(eng_words[j],french_words[i])]/ float(sum_delta2)
