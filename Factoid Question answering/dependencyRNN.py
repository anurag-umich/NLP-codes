...
class DependencyRNN:
    ...
    def __init__(self, d, V, r, answer_idxs, embeddings=None, seed=0):
	    ...
	    Normalizedtanh =  T.tanh(x)/T.sqrt(((T.tanh(x))**2).sum())

            return Normalizedtanh
            
            
        self.f = normalized_tanh
        def cost_calc (xc , xz ,h_n):
	    return T.maximum(0,1- T.dot(xc ,h_n) + T.dot(xz ,h_n))
        ...
        def recurrence(n, hidden_states, hidden_sums, cost, x, r, p, wrong_ans, corr_ans):
            ...
            state_update = T.set_subtensor(hidden_states[n] , h_n)
            state_sum_update = T.set_subtensor(hidden_sums[p[n]] , hidden_sums[p[n]] + T.dot(r[n],h_n))
 
            
            cost_vec ,updates = theano.scan(fn = cost_calc ,outputs_info = None, sequences = wrong_ans ,non_sequences = [corr_ans ,h_n] )
            total_cost = T.as_tensor_variable(cost_vec.sum())

            overall_node_cost = cost + total_cost
            return state_update , state_sum_update , overall_node_cost
